...
const App = () => {
  ...
  // HIGHLIGHT: This effect now runs ONCE on mount to get critical, non-language-specific data.
  useEffect(() => {
    const bootstrapApp = async () => {
        ...
        try {
            ...
            setIsLoading(false); // We set loading to false here
        }
    };
    bootstrapApp();
  }, []); // Empty dependency array ensures this runs only once.

  // HIGHLIGHT: This effect is now ONLY for fetching language-specific data (genres).
  useEffect(() => {
    const fetchGenres = async () => {
        try {
            ...
            // Don't set a main error, as the app can still function without genre names.
        }
    };
    fetchGenres();
  }, [language]); // It re-runs ONLY when the language changes.
  ...
  const fetchFullMovieDetails = useCallback(async (movieId, lang) => { /* ... No Changes from trailer-only version ... */ }, [userRegion]);
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 font-sans app-container relative">
        {/* The entire JSX body is restored here. */}
        ...
        {modalMovie && ( /* ... Unchanged ... */ )}
        {isTrailerModalOpen && movieDetails.trailerKey && ( /* ... Unchanged ... */ )}
        {isFilterModalOpen && ( /* ... Unchanged ... */ )}
                ...
                <div className="text-center max-w-md bg-[var(--color-card-bg)] p-8 rounded-xl shadow-2xl">
                    ...
                    {availableRegions.length > 0 ? (
                        <select 
                            ...
                            defaultValue=""
                            className="w-full p-3 bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg focus:ring-[var(--color-accent)] focus:border-[var(--color-accent)] text-[var(--color-text-primary)]"
                        >
