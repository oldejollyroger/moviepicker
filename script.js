...
const App = () => {
  ...
  if (!language) { return (<div className="min-h-screen bg-[var(--color-bg)] text-[var(--color-text-primary)] p-8 flex items-center justify-center"><div className="text-center max-w-md"><h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[var(--color-accent-gradient-from)] to-[var(--color-accent-gradient-to)] mb-6">Select Your Language</h1><div className="flex justify-center gap-4"><button onClick={() => handleLanguageSelect('es')} className="px-8 py-3 bg-[var(--color-card-bg)] hover:bg-[var(--color-accent)] rounded-lg font-bold text-lg transition-colors">Español</button><button onClick={() => handleLanguageSelect('en')} className="px-8 py-3 bg-[var(--color-card-bg)] hover:bg-[var(--color-accent)] rounded-lg font-bold text-lg transition-colors">English</button></div></div></div>); }
    ...
    <div className="min-h-screen p-4 sm:p-8 font-sans app-container relative">
                  ...
                  {isFetchingDetails ? <div className="flex justify-center"><div className="small-loader"></div></div> :  movieDetails.similar?.length > 0 ? ( 
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 place-items-center">
                          {movieDetails.similar.map(movie => ( 
                              <button key={movie.id} onClick={() => handleSimilarMovieClick(movie)} className="w-full text-center hover:scale-105 transition-transform duration-150 group">
      ...
      {modalMovie && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="bg-[var(--color-card-bg)] rounded-xl shadow-2xl max-w-2xl w-full p-6 relative">
            <button onClick={closeModal} className="absolute top-2 right-2 text-gray-400 hover:text-white text-2xl font-bold">&times;</button>
            {isFetchingModalDetails ? (
              ...
              <MovieCardContent movie={{
                ...
                year: modalMovie.release_date ? parseInt(modalMovie.release_date.split('-')[0]) : null,
                imdbRating: modalMovie.vote_average?.toFixed(1),
                genres: modalMovie.genres?.map(g => g.name) || [],
                poster: modalMovie.poster_path,
              }} details={modalMovie} isFetching={false} t={t} userRegion={userRegion} />
            ) : null}
          </div>
        </div>
      )}
      ...
      {isTrailerModalOpen && movieDetails.trailerKey && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
          <div className="relative w-full max-w-2xl">
            <button onClick={closeTrailerModal} className="absolute top-2 right-2 text-white text-3xl font-bold z-10">&times;</button>
