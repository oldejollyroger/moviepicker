...
const App = () => {
  ...
  const handleGenreChange = (genreId, type) => {
    setFilters(f => {
        ...
        if (index > -1) { newCurrentList.splice(index, 1); } else { newCurrentList.push(genreId); if (otherIndex > -1) { newOtherList.splice(otherIndex, 1); } }
  ...
  // Language selection is removed, so we check for country right away.
  if (!userRegion) {
    return (
