...
const App = () => {
  ...
  const fetchApi = useCallback(async (path, query) => {
    if (typeof TMDB_API_KEY === 'undefined' || !TMDB_API_KEY) { throw new Error("API Key is missing. Please ensure it's defined in your config file."); }
  ...
  useEffect(() => {
    ...
    const fetchNewMediaBatch = useCallback(async () => {
        ...
        const fetchPage = async (voteCount) => {
          ...
          const sortOptionsForVariety = ['popularity.desc', 'vote_average.desc'];
          const fetchPromises = sortOptionsForVariety.map(sortBy => {
            const randomPage = Math.floor(Math.random() * 20) + 1;
            return fetchApi(discoverPath, { ...queryParams, sort_by: sortBy, page: randomPage });
          });

          const results = await Promise.allSettled(fetchPromises);
          return results.reduce((acc, result) => {
            if (result.status === 'fulfilled' && result.value.results) { return acc.concat(result.value.results); }
            return acc;
          }, []);
        };
        
        try {
          let initialResults = await fetchPage(mediaType === 'movie' ? 100 : 50);
          ...
          const transformedMedia = uniqueResults
            .filter(media => media && media.overview && media.poster_path && (media.release_date || media.first_air_date))
            .map(media => normalizeMediaData(media, mediaType, genresMap));
          ...
          if (unwatchedMedia.length > 0) {
            ...
            setSessionShownMedia(prev => new Set(prev).add(newMedia.id));
          } else { setSelectedMedia(null); }
        } catch (err) { console.error("Error discovering media:", err); setError(err.message); setAllMedia([]); } 
        finally { setIsDiscovering(false); }
    }, [filters, language, mediaType, userRegion, genresMap, watchedMedia, selectedMedia, fetchApi]);

    const handleSurpriseMe = useCallback(() => {
        const availableMedia = allMedia.filter(m => !sessionShownMedia.has(m.id));
        if (availableMedia.length > 0) {
            const newMedia = availableMedia[Math.floor(Math.random() * availableMedia.length)];
            if (selectedMedia) setMediaHistory(prev => [...prev, selectedMedia]);
            setSelectedMedia(newMedia);
            setSessionShownMedia(prev => new Set(prev).add(newMedia.id));
        } else {
