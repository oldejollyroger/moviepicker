...
const App = () => {
    ...
    const fetchPage = async (voteCount) => {
        ...
        for (const result of results) {
            if (result.status === 'fulfilled' && result.value.ok) {
                ...
                const reason = result.reason || result.value?.statusText;
                console.warn(`A variety search failed:`, reason); 
            }
        }
        return allResults;
    };
  ...
  const fetchFullMovieDetails = useCallback(async (movieId, lang) => {
    try {
        ...
        let similarMovies = data.similar?.results
            ?.filter(m => m.poster_path && m.vote_average > 5 && m.id !== parseInt(movieId))
            .slice(0, 10) || [];
  ...
  useEffect(() => {
    ...
    if (storedWatched) {
      ...
      } catch (e) { console.error("Could not parse watched movies from localStorage", e); }
    }
  }, []);
    ...
    <div className="min-h-screen p-4 sm:p-8 font-sans app-container relative">
      ...
      <div className="text-center mb-10 flex justify-center items-center gap-4">
        ...
        <button 
            ...
            title={!userRegion ? t.selectRegionPrompt : ''}
