...
const App = () => {
    ...
    useEffect(() => { localStorage.setItem('mediaPickerFilters_v5', JSON.stringify(filters)); }, [filters]);
    ...
    return (
        <div className="min-h-screen p-4 font-sans app-container">
            ...
            <div className="max-w-4xl mx-auto mb-8 p-4 container-style">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4 items-center">
                    <div className="lg:col-span-1"><label htmlFor="decade-filter" className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">{t.decade}</label><select id="decade-filter" value={filters.decade} onChange={(e) => handleFilterChange('decade', e.target.value)} className="w-full p-2 bg-white/10 dark:bg-black/20 border border-slate-200 dark:border-slate-800 rounded-lg focus:ring-[var(--color-accent)] focus:border-[var(--color-accent)] text-[var(--color-text-primary)]"><option value="todos">{t.allDecades}</option>{[2020, 2010, 2000, 1990, 1980, 1970].map(d=>(<option key={d} value={d}>{`${d}s`}</option>))}</select></div>
                    <div className="lg:col-span-1"><label htmlFor="min-rating-filter" className="block text-sm font-medium text-[var(--color-text-secondary)] mb-1">{t.minRating} {Number(filters.minRating).toFixed(1)}</label><input type="range" id="min-rating-filter" min="0" max="9.5" step="0.5" value={filters.minRating} onChange={(e) => handleFilterChange('minRating', e.target.value)} className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-[var(--color-accent)]"/></div>
                    <div className="lg:col-span-2 grid grid-cols-2 gap-x-6">
                        <div><label htmlFor="min-age-filter" className="filter-label"><span className="text-sm font-medium text-[var(--color-text-secondary)]">{t.minAge}</span><span className="text-sm font-semibold text-[var(--color-accent-text)]">{ageRatingOptions[filters.minAge] || t.any}</span></label><input type="range" id="min-age-filter" min="0" max={ageRatingOptions.length - 1} step="1" value={filters.minAge} onChange={(e) => handleFilterChange('minAge', e.target.value)} className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-[var(--color-accent)]"/></div>
                        <div><label htmlFor="max-age-filter" className="filter-label"><span className="text-sm font-medium text-[var(--color-text-secondary)]">{t.maxAge}</span><span className="text-sm font-semibold text-[var(--color-accent-text)]">{ageRatingOptions[filters.maxAge] || t.any}</span></label><input type="range" id="max-age-filter" min="0" max={ageRatingOptions.length - 1} step="1" value={filters.maxAge} onChange={(e) => handleFilterChange('maxAge', e.target.value)} className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-[var(--color-accent)]"/></div>
                    </div>
                </div>
            </div>
